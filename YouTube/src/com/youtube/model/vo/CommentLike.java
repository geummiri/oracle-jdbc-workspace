package com.youtube.model.vo;

import java.util.Date;

public class CommentLike {

	private int commLikeCode;
	private Date commLikeDate;
	private Video video;  //COMMENT_CODE
	private Member member; //MEMBER_ID
	
	public CommentLike() {
	}

	public CommentLike(int commLikeCode, Date commLikeDate, Video video, Member member) {
		this.commLikeCode = commLikeCode;
		this.commLikeDate = commLikeDate;
		this.video = video;
		this.member = member;
	}

	public int getCommLikeCode() {
		return commLikeCode;
	}

	public void setCommLikeCode(int commLikeCode) {
		this.commLikeCode = commLikeCode;
	}

	public Date getCommLikeDate() {
		return commLikeDate;
	}

	public void setCommLikeDate(Date commLikeDate) {
		this.commLikeDate = commLikeDate;
	}

	public Video getVideo() {
		return video;
	}

	public void setVideo(Video video) {
		this.video = video;
	}

	public Member getMember() {
		return member;
	}

	public void setMember(Member member) {
		this.member = member;
	}

	@Override
	public String toString() {
		return "CommentLike [commLikeCode=" + commLikeCode + ", commLikeDate=" + commLikeDate + ", video=" + video
				+ ", member=" + member + "]";
	}
	
		
}
