 ...
 vs 정보(info) : 수집한 데이터를 어떠한 목적을 위해 분석하거나 가공하여
                새로운 의미 부여
 ...
 -SQL 종류
    1.DDL(Data Definition Language) : 데이터 정의어
        - DBMS의 구조를 정의하고나 변경, 삭제하기 위해 사용하는 언어
    ...
    4.DCL (Data Control Language) : 데이터 제어어
        -DBMS 에 대한 보안, 무결성, 복구 등 DBMS를 제어하기 위한 언어
        (GRANT 권한 부여, REVOKE 권한 회수)
    5.TCL(Transaction Control Langugae) :트랜젝션을 제어하는 언어
        (COMMIT실행, Rollback취소)
 *
 ...
 /*
    ...
    -데이터에서 데이터를 조회할 때 사용하는 SQL문
    조회된 결과를 RESULT set
    (즉 , 조회된 행들의 집합)
    -조회하고자 하는 컬럼들은 반드시 FROM 절에 기술한 테이블에 존재하는 컬럼이여야한다.
 ...
 */
 --EMPLOYEE 테이블에서 직원명(EMP_NAME), 직원의 연봉 조회
 ...
 */
 ...
 SELECT EMP_ID 사번, EMP_NAME AS 직원명, SALARY 급여, '원' AS "단위(원)"
 FROM EMPLOYEE;
 ...
 */
 ...
 SELECT EMP_ID || EMP_NAME || SALARY
 FROM EMPLOYEE;
 --xxx(사원명)의 월급은 XXX(급여)원입니다. "급여 정보"
 ...
 */
 ...
 --재직중(ENY_YN 컬럼값이 'N')인 사원들의 사번, 사원명, 입사일 조회
 ...
 /*
    ...
    [표현법]
    비교대상컬럼 BETWEEN 하한값(최하값) AND 상한값
 */
 
  --입사일이 '90/01/01' ~ '01/01/01' 인 사람들
...
[표현법]
비교대상컬럼 LIKE '특정패턴'
-특정팬턴에는 '%', '_'를 와일드 카드로 사용

'%' : 0글자 이상
비교대상컬럼 LIKE '문자%' => 비교대상컬럼값이 문자로 "시작"되는 것을 조회
비교대상컬럼 LIKE '%문자' => 비교대상컬럼값이 문자로 "끝"나는 것을 조회
비교대상컬럼 LIKE '%문자%' => 비교대상컬럼값에 문자가 "포함"되는 것을 조회
                            (키워드 검색)
'_' : 1글자
비교대상컬럼 LIKE '_문자' => 비교대상컬럼값 문자앞에 무조건 한글자가 올 경우 조회
비교대상컬럼 LIKE '__문자' => 비교대상컬럼값 문자앞에 무조건 두글자가 올 경우 조회
...
--ESCPAE OPTION : 나만의 와일드 카드를 만들어서 사용!

--이메일 중 _ 앞글자가 3글자인 사원들의 사번, 이름, 이메일 조회
--논리부정연산자 : NOT
--EAMIL앞뒤로 not 붙여도 상관없음
...
--보너스를 받지 않는 사원들의 사번 사번, 이름, 급여 ,보너스 조회
SELECT EMP_ID, EMP_NAME 사원명, SALARY 급여, BONUS 보너스
FROM EMPLOYEE
WHERE BONUS IS NULL;
...
SELECT EMP_NAME 사원명 , DEPT_CODE 부서, SALARY 급여
FROM EMPLOYEE
--WHERE DEPT_CODE NOT IN('D6', 'D8','D5'); 두가지 방법 있음
...
SELECT EMP_NAME 사원명 , MANAGER_ID 사수번호, DEPT_CODE 부서
FROM EMPLOYEE
...
--4. 급여가 200만원이상 500만원 이하고 입사일이 '01/01/01' 이상이고 보너스를 받지 않는 사원들의
...
--      사원들의 사번, 사원명, 급여, 보너스포함연봉 조회(별칭)
SELECT EMP_ID 사번, EMP_NAME 사원명, SALARY 급여,((SALARY*BONUS)+ SALARY)*12 보너스포함연봉, '원' AS "단위(원)"
FROM EMPLOYEE
...
/*
    ...
    ORDER BY 정렬하고자하는컬럼값 [ASC|DESC] [NULLS FIRST|NULLS LAST];
        ...
        -NULLS FIRST : 정렬하고자하는 컬럼값에 NULL이 있을 경우
                        해당 데이터를 맨 앞 배치 (생략 시 DESC일때 기본값)
        -NULLS LAST : 정렬하고자하는컬럼값에 NULL이 있을 경우
                        해당 데이터를 맨 뒤 배치(생략 시 ASC일때 기본값)              
*/

--전 사원의 사원명, 보너스 조희(보너스 기준 오름차순 정렬)
SELECT EMP_NAME, BONUS
FROM EMPLOYEE
WHERE BONUS IS NOT NULL
--ORDER BY BONUS;  ---보너스 기준 오름차순 정렬
--ORDER BY BONUS DESC; --보너스 기준 내림차순 정렬
ORDER BY BONUS DESC NULLS LAST; -- 보너스기준 내림차순 정렬(NULL이 맨뒤) 
