 /*
    ...
    : 데이터 조작 언어로 테이블에 값을 
     ...
     1)INSERT INTO 테이블명 VALUES(값, 값, 값, ...);
        -테이블 모든 컬럼에 대한 ㄱ밧을 INSERT 하고자 할 때 사용
     ...
     3)INSERT INTO 테이블명(서브쿼리);
        ...
        -서브쿼리의 결과값이 INSERT문에 지정된 테이블 컬럼의 개수와 데이터 타입이 같아야함
 */
 DROP TABLE EMP_01;
 CREATE TABLE EMP_01(
...
--모든 컬럼에 값을 지정하지않음
INSERT INTO EMP_01 
VALUES(200,'이병헌','개발 지원팀'); --에러

--에러는 발생하지 않지만 데이터 오기입
INSERT INTO EMP_01 
VALUES(300,'유아지원팀','공유',DEFAULT);  --가능

--컬럼 순서와 데이터 타입 불일치
INSERT INTO EMP_01
VALUES('유아지원팀',300,'위하준', DEFAULT); --에러
...
--EMP_NAME이 NOT NULL 제약조건에 의한 에러발생
...
--표현법3)
--EMPLOYEE 테이블에서 직원의 사번, 이름, 부서명, 입사일 조회 후
--EMP_01 테이블 INSERT 하기
...
/*
    ...
    INSERT ALL을 이용해서 여러 테이블에 한 번 데이터 삽입이 가능
    
    [표현법]
    1)INSERT ALL
        INTO 테이블명1[(컬럼, 컬럼, ...)] VALUSE(값, 값, ...)
        INTO 테이블명2[(컬럼, 컬럼, ...)] VALUSE(값, 값, ...)
        서브쿼리;
    2)INSERT ALL 
        WHEN 조건1 THEN
...
--예제
--EMP_DEPT 테이블에 EMPLOYEE 테이블의 부서 코드가 D1인 직원의
--사번, 이름, 부서코드 ,입사일 삽입
--EMP_MANAGER 테이블에 EMPLOYEE 테이블 부서코드가 D1인
--사번, 이름, 관리자 사번 삽입
...
INTO EMP_MANAGER VALUES(EMP_ID, EMP_NAME, MANAGER_ID)
    SELECT EMP_ID, EMP_NAME, DEPT_CODE, HIRE_DATE, MANAGER_ID --서브쿼리 넣깅!
    FROM EMPLOYEE
    WHERE DEPT_CODE = 'D1';
...
--2000년 1월 1일 이후 입사한 사원의 정보는 EMP_NEW 테이블에 삽입
...
/*
    ...
    -SET절에서 여러 개의 컬럼을 콤마(,)로 나열해서 값을 동시에 변경 가능
    -WHERE절을 생략하면 모든 행의 데이터 변경
    -UPDATE 시 서브쿼리를 사용하여 수행한 결과값으로 컬럼 값 변경 가능
*/
...
SET SALARY = 100
WHERE EMP_NAME = '노옹철';
--EMP_SALARY 테이블에서 선동일 급여 7000원  보너스 0.2 변경
UPDATE EMP_SALARY
SET SALARY = 7000, BONUS = 0.2
WHERE EMP_NAME = '선동일';

--모든 사원 급여를 기존 급여 10프로 인상으로 변경
UPDATE EMP_SALARY
...
--EMP_SALARY 테이블에서 아시아 지역에 근무하는 직원들의 보너스를 0.3변경
UPDATE EMP_SALARY
SET BONUS  = 0.3
...
ROLLBACK;
--컬럼은 삭제해도 롤백 안되나, 데이터는 가눙

--EMP_SALARY 테이블에서 DEPT_CODE D5인 직원들을 삭제
DELETE FROM EMP_SALARY
WHERE DEPT_CODE = 'D5';
