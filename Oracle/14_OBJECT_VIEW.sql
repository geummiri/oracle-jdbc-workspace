...
--VIEW 옵션
--2. FORCE | NOFORCE
--NOFORCE(기본값) : 서브쿼리에 기술된 테이블이 존재하는 테이블이여만 뷰가 생성됨

DROP TABLE TEST;

--테이블 또는 뷰가 존재하지 않는다는 에러 발생
--테이블 생성 후 만들어야 함
CREATE OR REPLACE NOFORCE VIEW VW_EMP --2.뷰 생성
AS SELECT * FROM TEST;

CREATE TABLE TEST( --1.테이블 먼저 만들고
    TID NUMBER
);
...
----WITH CHECK OPTION 사용 코드
CREATE OR REPLACE VIEW VW_EMP
AS SELECT *
    FROM EMPLOYEE
    WHERE SALARY >= 3000000
WITH CHECK OPTION; -->마지막단에 작성

UPDATE VW_EMP
SET SALARY = 2000000-->에러 발생
...
--INSERT
INSERT INTO VW_DEPT VALUES('D0','해외영업4부','L5');--에러/읽기전용뷰

--UPDATE
UPDATE VW_DEPT
SET LOCATION_ID = 'L2'
WHERE DEPT_TITLE = '해외영업2부';--에러/읽기전용뷰
