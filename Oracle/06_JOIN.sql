  /*
    ...
    - 부서 데이터는 부서테이블, 사원에 대한 데이터는 사원 테이블, 직급 테이블 ~~
        만약 어떤 사원이 어떤 부서에 속해있는지 
        부서명과 같이 조회하고 싶다면?
        만약 어떤 사원이 어떤 직급인지 직급명과 같이 조회하고 싶다면?
    
    => 관계형 테이터베이스에서 SQL문을 이용한 테이블 간에 "관계"를 맺어 원하는 데이터를 조회하는 방법
    
    -크게 "오라클 구문" 과 "ANSI 구문"
    -ANSI (미국국립표준협회 == 산업표준을 제정하는 단체) : 다른 DBMS 에서도 사용 가능
    
    1.오라클 - 등가 조인(EQUAL JOIN)
    /ANSI - 내부 조인(INNER JOIN/MATURAL JOIN)
    ...
    2) ANSI 표준 구문
        ...
        (USING 구문 / ON 구문 사용)
        -연결에 사용하려는 컬럼명이 같은 경우 ON 구문 대신에 USING(컬럼명) 구문을 사용
  */
  ...
  --사번, 사원명, 부서코드, 부서명을 같이 조회
    ...
    -- 오라클 구문
     ...
     WHERE DEPT_CODE = DEPT_ID; -- EMPLOYEE.DEPT_CODE = DEPARTMENT.DEPT_ID;
     -- ANSI 표준 구문
     SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE
     FROM EMPLOYEE
  ...
  --사번, 사원명, 직급코드, 직급명 조회
    ...
    SELECT EMP_ID, EMP_NAME, J.JOB_CODE, JOB_NAME
     ...
     FROM EMPLOYEE E, JOB J
     WHERE E.JOB_CODE = J.JOB_CODE  --!!먼저 조인하고 그다음에 AND 조건문 사용해줘야함!!
     AND JOB_NAME='대리';
...
*/
--사원명, 부서명, 급여, 연봉 조회
--부서 배치가 아직 안된 사우너 2명에 대한 정보 조회 X
...
SELECT EMP_NAME 사원명, DEPT_TITLE 부서명, SALARY 급여, SALARY * 12 연봉  
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); --23명 조회 (부서가 NULL인 곳도 조회 가능)
...
SELECT EMP_NAME 사원명, DEPT_TITLE 부서명, SALARY 급여, SALARY * 12 연봉
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+); --EMPLOYEE 테이블을 기준으로 반대편에 해당하는 컬럼 뒤에 (+) 입력
...
--사원이 NULL인 경우, 부서가 NULL인 경우 모두 조회 가능

--> 오라클 구문 (오라클 구문은 FULL JOIN 사용 불가능) 에러 발생!!!!
--SELECT EMP_NAME 사원명, DEPT_TITLE 부서명, SALARY 급여, SALARY * 12 연봉
--FROM EMPLOYEE, DEPARTMENT
--WHERE DEPT_ID(+) = DEPT_CODE(+);
...
/*
    ...
    ANSI 구문으로는 JOIN ON 으로만 사용 가능! (USING 사용불가)
*/
...
*/
--사원사번, 사원명, 사원부서코드, 사수사번, 사수명, 사수부서코드조회
--EMP_ID 랑 MANAGER_ID 동일하면 
--별칭 이용해서????
SELECT * FROM EMPLOYEE;

-->> 오라클 구문
SELECT E.EMP_ID 사원사번, E.EMP_NAME 사원명, E.DEPT_CODE 사원부서코드,
     M.EMP_ID 사수사번, M.EMP_NAME 사수명, M.DEPT_CODE 사수부서코드조회
FROM EMPLOYEE E, EMPLOYEE M -- EMPLOYEE E 사원, EMPLOYEE M  사수
...
FROM EMPLOYEE
CROSS JOIN DEPARTMENT; -- 사원 23 * 부서 9 = 207
-->> 오라클 구문
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT;
...
--(모든 테이블 다 조인)
-->> 오라클 구문
SELECT EMP_NO, EMP_NAME, DEPT_TITLE, JOB_NAME, NATIONAL_NAME, SAL_LEVEL
...
SELECT EMP_NO, EMP_NAME, DEPT_TITLE, 
       JOB_NAME, NATIONAL_NAME, SAL_LEVEL
FROM EMPLOYEE E
...
SELECT NVL(DEPT_TITLE, '부서없음') 부서명, TO_CHAR(FLOOR(AVG(SALARY)),'9,999,999') 급여평균
...
--이때 구분에 해당하는 값은 고급,중급,초급
--급여 등급이 S1,2 고급 3,4 중급 5,6, 초급
