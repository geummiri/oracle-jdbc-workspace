...
SELECT NVL(DEPT_CODE, '부서없음') "부서", 
        COUNT(*) "사원 수", COUNT(BONUS) "보너스 받는 사원 수", 
        SUM(SALARY) "총 급여",
...
-- (부서코드, 평균급여)
SELECT DEPT_CODE, FLOOR(NVL(SALARY,0))
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
--컬럼이 하나일 때는 사실상 CUBE, ROLLUP 차이점이 없음

--부서 코드와 직급 코드가 같은 사원들의 급여 합계 조회
SELECT DEPT_CODE, JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE -- 그룹바이 두개도 가넝
ORDER BY DEPT_CODE;
...
--AND
--4.MINUS
--부사 코드가 D5인 사원들 중에서 급여가 300만원 초과인 사원들을 제외해서 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
MINUS
