...
*/
  ...
  --HELLO ORABLE 출력
  BEGIN 
    DBMS_OUTPUT.PUT_LINE('HELLO ORACLE'); --HELLO ORACLE 출력됨
    END;
    /
  ...
  */ 
  DECLARE
    EID NUMBER;
    ENAME VARCHAR2(20);
    PI CONSTANT NUMBER := 3.14; --상수값 초기화 할 때 := 이렇게 꼭 작성해주기
 BEGIN
    EID := &번호;
    ENAME := '&이름';
...
--문제로 풀어보기!
--레퍼런스 타입으 변수로 EID, ENAME, JCODE, DTITLE, SAL를 선언하고
--각 변수의 자료형은 EMPLOYEE 테이블의 EMP_ID, EMP_NAME, JOB_CODE,
--SALARY컬럼과 DEPARTMENT 테이블의 DEPT_TITLE 컬럼의 자료형을 참조
 ...
 */
 --사번을 입력받은 후 해당 사원의 사번(EID), 이름(ENAME), 급여(SAL), 보너스(BONUS) 출력
 --단, 보너스를 받지 않은 사원은 보너스 출력전에
...
/*
    ...
    END IF;
*/
SET SERVEROUTPUT ON; --화면단에 띄울수있게끔 처리하는 기능
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    SAL EMPLOYEE.SALARY%TYPE;
...
--일반타입변수 TEAM을 데이터 타입 VARCHAR2(10)으로 선언
--NCODE 값이 KO일 경우 --> TEAM '국내팀' 아닐경우 '해외팀
...
/*
    ...
    .. 
    ELSEIF (ELSEIF 여러개 가능)
    ELSEIF
    ..
    [ELSE 실행내용N] 필수값 아님
    END IF;
*/
--사용자에게 점수를 입력받아 SCORE 변수에 저장한 후 학점은 입력된 점수에 따라 GRADE 변수 저장
--90점이 이상 'A' 80이상 'B' 70점 이상 'C' 60점 이상 'D' 60점 미만 'F'
--출력  '당신의 점수는 95점이고, 학점은 A학점입니다.' 와 같이 출력
...
--출력은 '해당사원의 급여 등급은 고급입니다.' 와 같이 출력
...
BEGIN
    ...
    INTO GRADE
    FROM SAL_GRADE
    WHERE SAL BETWEEN MIN_SAL AND MAX_SAL; --위 문제랑 다른점
...
--D1인 경우 인사관리부, D2 회계관리부, D3인 경우 마케팅부,D4인 경우 국내영업부, 
--D5인 경우 해외영업1부, D6인 경우 해외영업2부, D7인 경우 해외영업3부, D8 기술지원부, D9 총무부 
--나머지는 부서없음
--출력은 사번, 이름, 부서코드, 부서 
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('사번 : ' || EMP.EMP_ID);
    DBMS_OUTPUT.PUT_LINE('이름 : ' || EMP.EMP_NAME);
    DBMS_OUTPUT.PUT_LINE('부서 : ' || EMP.DEPT_CODE);
...
/*
    2.반복문 (1-BASIC LOOP , 2-FOR LOOP) 반복문은 BEGIN에 넣기
    2-1)BASIC LOOP
...
BEGIN
    LOOP
        DBMS_OUTPUT.PUT_LINE(NUM); --반복문 실행할 구문
        NUM := NUM+1; --1씩 증가
        IF(NUM > 5) THEN EXIT; --반복문 빠져나가는 구문
        END IF; --IF문 빠져나가고
    END LOOP; --LOOP문도 빠져나가기
END;
/
...
BEGIN
    LOOP
        DBMS_OUTPUT.PUT_LINE(NUM); --반복문 실행할 구문
        NUM := NUM+1; --1씩 증가
        EXIT WHEN NUM >5 ; --반복문 빠져나가는 구문
    END LOOP; --LOOP문 빠져나가기
END;
/ 
/*  반복문은 BEGIN에 넣기
    2-2)FOR LOOP
...
--TEST테이블에 10개의 행을 INSERT 하는 PL/SQL 작성
...
/*
    ...
    *오라클에서 미리 정의되어 있는 시스템 예외
    ...
    -TOO_MANY_ROWS : SELECT 한 결과가 한 행이 리턴되어야 하는데 여러 행일 경우
...
BEGIN
    DBMS_OUTPUT.PUT_LINE(10/&숫자); --0을 입력한 경우 EXCEPTION 해줘야함
EXCEPTION
    WHEN ZERO_DIVIDE 
...
BEGIN
    UPDATE EMPLOYEE
    SET EMP_ID ='203'
    WHERE EMP_NAME = '&이름';--기존 사번이 있을 경우 위배 됨
EXCEPTION
...
EXCEPTION
    ...
    WHEN OTHERS --이 외의 것들
    THEN DBMS_OUTPUT.PUT_LINE('에러가 발생했습니다. 관리자에게 문의해주세요.');
END;
/
