...
--노옹철 사원의 급여보다 더 많이 받는
--사원의 사번, 사원명, 부서명, 직급코드, 급여조회
 
SELECT EMP_NO, EMP_NAME, DEPT_TITLE, JOB_CODE, SALARY
...
FROM EMPLOYEE
GROUP BY DEPT_CODE 
HAVING SUM(SALARY) =  (SELECT MAX(SUM(SALARY)) --급여 합의 최대값
                        FROM EMPLOYEE
                        GROUP BY DEPT_CODE);
...
--전지연 사원이 속해있는 부서원들의
--사번, 사원명, 전화번화, 직급명, 부서명, 입사일 조회(단, 전지연 사원은 제외)
SELECT EMP_NO, EMP_NAME, PHONE, DEPT_TITLE, HIRE_DATE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_ID = DEPT_CODE
AND EMP_NAME != '전지연'
AND DEPT_TITLE
= (SELECT DEPT_TITLE FROM DEPARTMENT, EMPLOYEE
    WHERE DEPT_ID = DEPT_CODE
    AND EMP_NAME ='전지연');
    
/*
    2.다중행 서브쿼리
    - 서브쿼리 조회 결과 값이 개수가 여러 행 일 때(여러행 한열)
...
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
                    FROM EMPLOYEE
                    WHERE EMP_NAME = '하이유'); --(('D5', 'J5')) 이것도 가능
...
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
GROUP BY  EMP_ID, EMP_NAME, JOB_CODE, SALARY
HAVING (JOB_CODE,SALARY) IN (SELECT JOB_CODE, MIN(SALARY) FROM EMPLOYEE GROUP BY JOB_CODE)
ORDER BY 3;
...
*/
--전 직원 중 급여가 가장 높은 상위 5명 순위, 이름, 급여조회
...
SELECT RANK() OVER(ORDER BY SALARY DESC),
        EMP_NAME,
        SALARY
FROM EMPLOYEE; --19위가 2명이여서 20위를 세지 않고 21위로 넘어감


SELECT DENSE_RANK() OVER(ORDER BY SALARY DESC),
        EMP_NAME,
        SALARY
FROM EMPLOYEE; --19위가 2명이여도 다음 순위는 20위

--상위 5명만 조회
SELECT RANK, --별칭 사용하기 ROWNUM 사용안됨
        EMP_NAME,
        SALARY
FROM (
    SELECT
        RANK() OVER(ORDER BY SALARY DESC) "RANK", --별칭 지정
        EMP_NAME,
        SALARY
    FROM EMPLOYEE
        )
